...
namespace TTF_Tree {
	void Test() {
		int key;
		vector<string> ops = { "ins 10", "show", "ins 20", "show", "ins 15", "show", "ins 32", "show", "ins 65", "show", "ins 43", "show", "ins 24", "show", "ins 54", "show"};
		TwoTreeFourTree t;
		int n = ops.size();
		for (string op : ops) {
			cout << op << endl;
			...
			else if (op.find("ins") < n)
			{
				t.insert_key(stoi(op.substr(op.find(" ") + 1)));
			}
			else if (op.find("del") < n)
			{
				t.delete_key(stoi(op.substr(op.find(" ") + 1)));
			}
		}
	}
};